...
public class SimpleAssembler implements Assembler
{
    ...
    public int assemble(String inputFileName, String outputFileName, StringBuilder error)
    {
        ...
        {
            lists = lines.map(line -> line.trim()).filter(line -> line.length() > 0).peek(line ->
            {
        ...
        List<Instruction> outputCode = lists.get(true).stream().map(line -> line.split("\\s+"))
                .map(this::makeCode).collect(Collectors.toList());

        List<DataPair> outputData = lists.get(false).stream().map(line -> line.split("\\s+"))
                .map(this::makeData).collect(Collectors.toList());
    ...
    public static void main(String[] args)
    {
        ...
        try (Scanner keyboard = new Scanner(System.in))
        {
            ...
            int i = new SimpleAssembler().assemble(filename + ".pasm", filename + ".pexe", error);
